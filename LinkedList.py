class Linked_List:
    ...
    def append_element(self, val):
        ...
        # of loops.
        new_node = self.__Node(val) # creates new node for element
        if self.__size == 0:
            self.__head = new_node
            self.__tail = new_node 
        else:
            self.__tail.next = new_node
            self.__tail = new_node # ensures element appends to end of list
    ...
    def insert_element_at(self, val, index):
        ...
        if index >= self.__size:
            raise IndexError
        if self.__size == 0:
            raise IndexError # prevents appending element to empty list
        new_node = self.__Node(val) #creates new node for element
        if index == 0:
            new_node.next = self.__head
            self.__head = new_node # uses header node if index is start of list
        else:
            current = self.__head
    ...
    def remove_element_at(self, index):
        ...
        if index == 0:
            ...
            if self.__size == 1:
                self.__tail = None # establishes end of list
        else:
            current = self.__head
            for i in range(1, index):
                current = current.next
    ...
    def __str__(self):
        ...
        while current is not None:
            ...
            current = current.next
        return "[ " + ", ".join(string) + " ]" # returns in proper format
